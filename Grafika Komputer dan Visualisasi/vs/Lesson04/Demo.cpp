...
void Demo::Init() {
	...
	BuildColoredRefrigeratorDoor();

	BuildColoredPaint();

	BuildColoredPaintPicture();
...
void Demo::Render() {
	...
	DrawColoredRefrigeratorDoor();

	DrawColoredPaint();

	DrawColoredPaintPicture();
...
void Demo::BuildColoredRefrigeratorDoor() {
	// load image into texture memory
	// ------------------------------
	// Load and create a texture 
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	int width, height;
	unsigned char* image = SOIL_load_image("RefrigeratorDoor2.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
...
void Demo::DrawColoredRefrigeratorDoor()
{
	glUseProgram(shaderProgram);
...
void Demo::BuildColoredPaintPicture() {
	// load image into texture memory
	// ------------------------------
	// Load and create a texture 
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	int width, height;
	unsigned char* image = SOIL_load_image("PaintPicture.png", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
...
void Demo::DrawColoredPaintPicture()
{
	glUseProgram(shaderProgram);
